 ...
 */
public class MinimaxSimulation extends Simulation {

    protected MinimaxSimulation(Graph graph) {
        super(graph);
    }
    ...
    public Tuple alphabeta(JSONObject state, int depth, int alpha, int beta, String agentId){
        ...
        if (agent.getString(AT).equals(agent.getString(GOAL))){
            return new Tuple(1, agent.getString(AT));
        }
        // if the other agent is at the goal - he loses
        if(otherAgent.getString(AT).equals(otherAgent.getString(GOAL))){
            return new Tuple(-1, agent.getString(AT));
        }
